...
export function shardUpgradeMarketingRequirement(upgradeId) {
  const upgrade = shardUpgradeById(upgradeId);
  const minResearchBuyMarketing = Math.max(
    0,
    Number(RESEARCH_UNLOCK_MARKETING || 0) + 1,
  );
  return Math.max(
    minResearchBuyMarketing,
    Number(upgrade?.minMarketing || minResearchBuyMarketing),
  );
}
...
export function marketBonusForHarvest(state, tile, seedId) {
  if (!hasToolUpgradesUnlocked(state)) return 0;
...
export function createInitialFarmSizeUnlocks() {
  const out = {};
  for (const exp of FARM_EXPANSIONS) out[exp.size] = exp.size === 1;
  return out;
}
...
export function farmExpansionMoneyCost(size) {
  const exp = farmExpansionBySize(size);
  return Math.max(0, Number(exp?.unlockMoney || 0));
}

export function farmExpansionShardCost(size) {
  const exp = farmExpansionBySize(size);
  return Math.max(0, Number(exp?.unlockShards || 0));
}

export function nextLockedMoneyFarmExpansion(state) {
  for (const exp of FARM_EXPANSIONS) {
    const unlockMoney = farmExpansionMoneyCost(exp.size);
    if (unlockMoney <= 0) continue;
    if (!state?.farmSizeUnlocks?.[exp.size]) return exp;
  }
  return null;
}

function normalizeFarmSizeUnlocks(unlocks, activeFarmSize = 1) {
  const out = { ...(unlocks || {}) };
  out[1] = true;
  const unlockedSizes = FARM_EXPANSIONS.map((exp) => exp.size).filter(
    (size) => Boolean(out[size]),
  );
  const maxUnlockedByData = Math.max(
    1,
    Number(activeFarmSize || 1),
    ...(unlockedSizes.length > 0 ? unlockedSizes : [1]),
  );
  for (const exp of FARM_EXPANSIONS) {
    if (exp.size <= maxUnlockedByData) out[exp.size] = true;
  }
  return out;
}

export function hasToolUpgradesUnlocked(state) {
  return Boolean(state?.farmSizeUnlocks?.[3]);
}

function maxSeedUnlockIndexForState(state) {
  // Early progression keeps seed choices intentionally small until farm growth.
  if (!state?.farmSizeUnlocks?.[2]) return 1;
  if (!state?.farmSizeUnlocks?.[3]) return 2;
  return Number.POSITIVE_INFINITY;
}
...
export function createInitialState(carry = {}) {
  ...
  const normalizedFarmSizeUnlocks = normalizeFarmSizeUnlocks(
    farmSizeUnlocks,
    Number(carry.activeFarmSize || 1),
  );
  const unlockedSizes = FARM_EXPANSIONS.map((exp) => exp.size).filter(
    (size) => normalizedFarmSizeUnlocks[size],
  );
  const maxUnlocked = unlockedSizes.length > 0 ? Math.max(...unlockedSizes) : 1;
  ...
  const rawHouseCount =
    carry.houseCount ??
    (carry.housePurchased || carry.postWinContinued ? 1 : 0);
  const houseCount = clamp(Math.floor(Number(rawHouseCount || 0)), 0, 2);
  const housePurchased = houseCount > 0 ? Boolean(carry.housePurchased) : false;
  return {
    version: 3,
    ...
    shardUpgrades,
    farmSizeUnlocks: normalizedFarmSizeUnlocks,
    activeFarmSize,
    ...
    houseCount,
    housePurchased,
    housePurchasedAt: housePurchased ? Number(carry.housePurchasedAt || 0) : 0,
    postWinContinued: Boolean(carry.postWinContinued || houseCount > 0),
    seedDemand,
...
function emitRuntimeEvent(state, payload) {
  const sink = state?.__runtimeEventSink;
  if (typeof sink !== "function") return;
  const seq = Math.max(0, Number(state?.__runtimeEventSeq || 0)) + 1;
  state.__runtimeEventSeq = seq;
  sink({
    at: Date.now(),
    mutationId: state?.__runtimeMutationId || null,
    eventSeq: seq,
    ...payload,
  });
}
...
export function normalizeState(raw) {
  ...
  const migratedHouseCount = clamp(
    Math.floor(
      Number(
        raw.houseCount ??
          (raw.housePurchased || raw.postWinContinued ? 1 : 0),
      ),
    ),
    0,
    2,
  );
  next.houseCount = migratedHouseCount;
  next.housePurchased = migratedHouseCount > 0 && Boolean(raw.housePurchased);
  next.housePurchasedAt = next.housePurchased
    ? Math.max(0, Number(raw.housePurchasedAt || 0))
    : 0;
  next.postWinContinued = Boolean(raw.postWinContinued || migratedHouseCount > 0);
  ...
  next.farmSizeUnlocks = normalizeFarmSizeUnlocks(
    next.farmSizeUnlocks,
    Number(raw.activeFarmSize || 1),
  );
  const unlockedSizes = FARM_EXPANSIONS.map((exp) => exp.size).filter(
...
export function harvestTile(state, tile) {
  ...
  const jackpot =
    ...
    upgrades.jackpot;
  const hitJackpot = Math.random() < jackpot;
  if (hitJackpot) {
    gain *= 2;
  }
  ...
  const creditedGain = Math.max(1, Math.floor(gain));
  state.money += creditedGain;
  ...
  if (marketBonus > 0) {
    emitRuntimeEvent(state, {
      kind: "bonus",
      source: "seasonal",
      amount: Math.max(1, Math.floor(creditedGain * marketBonus)),
      count: 1,
    });
  }
  ...
  const regrowTriggered = isMature && Math.random() < regrowChance;
  if (regrowTriggered) {
    emitRuntimeEvent(state, {
      kind: "bonus",
      source: "regrow",
...
export function isToolVisible(state, toolId) {
  ...
  if (toolId === "research") {
    if (!hasToolUpgradesUnlocked(state)) return false;
...
export function updateDiscoveries(state) {
  ...
  const maxSeedUnlockIndex = maxSeedUnlockIndexForState(state);
  const toolUpgradesUnlocked = hasToolUpgradesUnlocked(state);

  // Seeds unlock progressively by tier order.
  for (let i = 0; i < SEED_UNLOCK_ORDER.length; i += 1) {
    if (i > maxSeedUnlockIndex) continue;
  ...
  if (
    !state.discovered.tools.expandFarm &&
    (state.activeFarmSize > 1 || money > 0)
  ) {
...
export function prestigeShardGain(state) {
  ...
  const moneyDamp = 14 + Math.log10(money + 10) * 2;
  const moneyPart = Math.floor(Math.sqrt(money) / moneyDamp);
  const harvestPart = Math.floor(Math.max(0, state.totalHarvests) / 10);
  const prestigePart = Math.floor(prestigeLevel / 2);
  const earlyMomentum = Math.max(1, Math.floor((prestigeLevel + 1) * 0.68));
  const baseGain = moneyPart + harvestPart + prestigePart + earlyMomentum;
  const lateMomentum =
    prestigeLevel >= 5 ? Math.floor((prestigeLevel - 4) * 0.95) : 0;
  const lateMultiplier =
    prestigeLevel >= 8 ? 1 + Math.min(0.28, (prestigeLevel - 7) * 0.022) : 1;
  return Math.max(3, Math.floor((baseGain + lateMomentum) * lateMultiplier));
}

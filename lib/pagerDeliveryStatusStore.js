...
const MEMORY_TEXT_INDEX_KEY = "__pagerDeliveryStatusTextIndex";
...
function getTextIndexStore() {
  if (!globalThis[MEMORY_TEXT_INDEX_KEY]) {
    globalThis[MEMORY_TEXT_INDEX_KEY] = new Map();
  }
  return globalThis[MEMORY_TEXT_INDEX_KEY];
}

function pruneStore(map) {
  const cutoff = Date.now() - RECORD_TTL_MS;
  const textIndex = getTextIndexStore();
  for (const [key, value] of map.entries()) {
    const acceptedAtMs = new Date(value?.acceptedAt || 0).getTime();
    if (!Number.isFinite(acceptedAtMs) || acceptedAtMs < cutoff) {
      map.delete(key);
      for (const [textKey, indexedTrackingKey] of textIndex.entries()) {
        if (indexedTrackingKey === key) {
...
async function kvGetMany(keys) {
  ...
  const rows = [];
  for (const chunk of chunkArray(safeKeys, 250)) {
...
function getKvTextIndexKey(textKey) {
  const normalizedTextKey = normalizeTextForMatch(textKey);
  if (!normalizedTextKey) return "";
  const hash = createHash("sha1")
    .update(normalizedTextKey, "utf8")
    .digest("hex")
    .slice(0, 24);
  return `pager:status:v1:text:${hash}`;
}
...
function isPendingRecordMatch(record, cutoff, textKey = "") {
  if (!record) return false;

  const acceptedMs = toEpochMs(record.acceptedAt);
  if (!acceptedMs || acceptedMs < cutoff) return false;

  const completedAt = String(record?.stages?.mmdvm_tx_completed?.at || "").trim();
  if (completedAt) return false;
  if (textKey && normalizeTextForMatch(record.text) !== textKey) return false;
  return true;
}

function upsertMemoryTextIndex(record) {
  const textKey = normalizeTextForMatch(record?.text);
  const trackingKey = String(record?.trackingKey || "").trim();
  if (!textKey || !trackingKey) return;
  getTextIndexStore().set(textKey, trackingKey);
}

async function kvGetRecordsByTrackingKeys(trackingKeys) {
  const uniqueTrackingKeys = [
    ...new Set(
      (Array.isArray(trackingKeys) ? trackingKeys : [])
        .map((key) => String(key || "").trim())
        .filter(Boolean),
    ),
  ];
  if (uniqueTrackingKeys.length <= 0) return [];

  const kvRecordKeys = uniqueTrackingKeys.map((trackingKey) =>
    getKvRecordKey(trackingKey),
  );
  const rows = await kvGetMany(kvRecordKeys);
...
async function kvPutRecord(record) {
  ...
  const textKey = normalizeTextForMatch(normalized.text);
  const textIndexKey = getKvTextIndexKey(textKey);
  ...
  if (textIndexKey) {
    await kv.set(textIndexKey, normalized.trackingKey, { ex: RECORD_TTL_SEC });
  }
  return normalized;
}
...
async function kvFindMostRecentPending(maxAgeMs, expectedTextNormalized = "") {
  ...
  if (textIndexKey) {
    const indexedTrackingKey = String(await kv.get(textIndexKey) || "").trim();
    if (indexedTrackingKey) {
      const indexedRecord = await kvGetRecordByTrackingKey(indexedTrackingKey);
      if (isPendingRecordMatch(indexedRecord, cutoff, textKey)) {
  ...
  const records = await kvGetRecordsByTrackingKeys(trackingKeys);
  let newest = null;
  let newestMs = 0;

  for (const record of records) {
    if (!isPendingRecordMatch(record, cutoff, textKey)) continue;
    const acceptedMs = toEpochMs(record.acceptedAt);
...
export async function getMostRecentPendingPagerStatusByText(
  ...
  const indexedTrackingKey = String(textIndex.get(textKey) || "").trim();
  if (indexedTrackingKey) {
    const indexed = store.get(indexedTrackingKey) || null;
    if (isPendingRecordMatch(indexed, cutoff, textKey)) {
      return indexed;
    }
    textIndex.delete(textKey);
  }
  ...
  for (const value of store.values()) {
    if (!isPendingRecordMatch(value, cutoff, textKey)) continue;
    const acceptedMs = toEpochMs(value?.acceptedAt);
    const sortMs = Math.max(toEpochMs(value?.updatedAt), acceptedMs);
    if (!newest || sortMs > newestMs) {
      newest = value;
  ...
  if (newest) {
    textIndex.set(textKey, newest.trackingKey);
  }
  return newest;
}
